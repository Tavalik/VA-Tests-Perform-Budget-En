...
Сценарий: Я создаю номенклатуру с именем 'TheProductName' родителем 'TheProductParent' видом номенклатуры 'TheProductKind' артикулом 'TheProductID'
	...
	* Удаляем текущий элемент
		Если '$$ЭтоУХ$$' Тогда
			...
			И Пока в таблице 'Список' количество строк 'больше' 0 Тогда
				И в таблице 'Список' я выбираю текущую строку
				И Я удаляю текущий элемент в окне "* (Product range)" очищая 'Артикул'
		ИначеЕсли '$$ЭтоЕРПУХ$$' Тогда
			И пауза 1
...
Сценарий: Я создаю внешнюю информационную базу 'TheName' тип 'TheType' каталог 'TheCatalog' тип хранилища 'TheTypeOfStorage'
	...
	* Создаем новый элемент			
		...
		Если '"[TheTypeOfStorage]" = "Excel"' Тогда 
			И я меняю значение переключателя с именем 'ТипХранилищаДанныхADO' на "MS Excel (.xls) files"
		...
		Если '"[TheTypeOfStorage]" = "FoxPro"' Тогда
			И я меняю значение переключателя с именем 'ТипХранилищаДанныхADO' на "FoxPro (.dbf) files"
			И в поле с именем 'ЭталонныйКаталог' я ввожу текст '[TheCatalog]'
		Если '"[TheTypeOfStorage]" = "Access"' Тогда
			И я меняю значение переключателя с именем 'ТипХранилищаДанныхADO' на "MS Access databases (.mdb)"
			И в поле с именем 'ЭталонныйКаталог' я ввожу текст '[TheCatalog]'
		И я запоминаю строку "[TheName] (External infobases)*" в переменную 'ЗаголовокОкна'
		И я нажимаю на кнопку с именем 'ФормаЗаписать'
