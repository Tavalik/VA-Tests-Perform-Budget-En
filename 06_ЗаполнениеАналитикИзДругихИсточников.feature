		...
		* Другой источник
			* Источник с прозвольным запросом №1
				...
				* Источник для дополнения аналитик
					...
					И я нажимаю на кнопку с именем 'ФормаСоздать'
					Тогда открылось окно "Data source (create)"
					И из выпадающего списка с именем 'ВидОтчетаОтбор' я выбираю по строке "VA - Data to add analytics"
		...
		* Рассчитываемый источник
			* Произвольный запрос к БД с многопериодным контекстом
				...
				* Источник для заполненния Анилитики3
					...
					И я нажимаю на кнопку с именем 'ФормаСоздать'
					Тогда открылось окно "Data source (create)"
					И из выпадающего списка с именем 'ВидОтчетаОтбор' я выбираю по строке "VA - Analysts from fields of other sources"
					И я нажимаю кнопку выбора у поля с именем 'ПоказательОтбор'
		...
		* Пересчет в коде
			* Основной источник
				...
				* Устанавливаем отборы
					...
					И в таблице 'ДеревоПараметровОтбораБД' из выпадающего списка с именем 'УточнениеСпособаОпределения' я выбираю по строке "5C:Corporate performance management"
			...
			* Вводим формулу расчета
				...
				Если '$$ЯзыкИнтерфейса$$ = "Ru"' Тогда					
					Если '$$ЭтоPerform$$' Тогда
						И в поле с именем 'ПолеТекстовогоДокументаПроцедура' я ввожу текст 'Результат = Макс([IR_ЦеныНомен_Price_], 1500000)'
					Иначе
						И в поле с именем 'ПолеТекстовогоДокументаПроцедура' я ввожу текст 'Результат = Макс([BA_ЦеныНомен_Price_], 1500000)'
				Иначе
