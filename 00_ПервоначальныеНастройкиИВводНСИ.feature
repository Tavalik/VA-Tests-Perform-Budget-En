...
Сценарий: 00.04 Создание организаций и ЦФО
	...
	* Создаем организацию Венера
		...
		Если '$$ЭтоPerform$$' Тогда
			И Я для организации "Venus LLC" для реквизита 'ГруппаКонтрагента' выбираю значение "VA - Subsidiary"
		Иначе			
			И Я для организации "Venus LLC" для реквизита 'ГруппаКонтрагентов' выбираю значение "VA - Subsidiary"
	...
	* Создаем организацию Земля
		...
		Если '$$ЭтоPerform$$' Тогда	
			И Я для организации "Earth LLC" для реквизита 'ГруппаКонтрагента' выбираю значение "VA - Subsidiary"
		Иначе			
...
Сценарий: 00.16 Создание Статей движения денежных средств
	...
	И Я создаю статью ДДС с именем "Software sale" родителем "VA - Cash flow items"
	И Я создаю статью ДДС с именем "Software implementation" родителем "VA - Cash flow items"
	И Я создаю статью ДДС с именем "Software upgrade" родителем "VA - Cash flow items"

Сценарий: 00.17 Создание Статьей доходов и расходов

	И Я создаю группу статей ДиР с именем 'ВА - Статьи доходов и расходов'
	И Я создаю статью ДиР с именем "Software sale" родителем 'ВА - Статьи доходов и расходов'
	И Я создаю статью ДиР с именем "Software implementation" родителем 'ВА - Статьи доходов и расходов'
	...
	Если 'НЕ $$ЭтоPerform$$' Тогда
		...
		* Создаем новый документ №1
			...
			* Бюджет движения денежных средств
				...
				И в таблице 'ТабБюджетДвиженияДенежныхСредств' из выпадающего списка с именем 'ТабБюджетДвиженияДенежныхСредствСтатьяДвиженияДенежныхСредств' я выбираю по строке "Software sale"
				...
				И в таблице 'ТабБюджетДвиженияДенежныхСредств' из выпадающего списка с именем 'ТабБюджетДвиженияДенежныхСредствСтатьяДвиженияДенежныхСредств' я выбираю по строке "Software implementation"
			...
			* Запишем документ, проверим движения
				...
				Если '$$ЭтоУХ$$' Тогда
					...
					Тогда таблица 'ДвиженияДенежныхСредствУХ' стала равной:
						| 'N' | "Cash flow item" | "Bank account / касса" | "Income expense" | "Kind денежных средств" | "Counterparty, подотчетник, касса ККМ" | 'Аналитика2' | "Counterparty contract"       | 'Аналитика1' | 'Аналитика3' | 'Аналитика4' | 'Аналитика5' | 'Аналитика6' | "Amount упр. учета" | 'Сумма'        |
						| '1' | "Software sale" | ''                        | 'Приход'        | 'Безналичная'          | 'ООО "Ганимед"'                      | ''           | 'Ганимед-001 от 01.01.2021' | ''           | ''           | ''           | ''           | ''           | '780,000.00'       | '780,000.00'   |
						| '2' | "Software implementation"  | ''                        | 'Приход'        | 'Безналичная'          | 'ООО "Ганимед"'                      | ''           | 'Ганимед-002 от 01.01.2021' | ''           | ''           | ''           | ''           | ''           | '2,340,000.00'     | '2,340,000.00' |
				Если '$$ЭтоЕРПУХ$$' Тогда
					Тогда таблица 'ФактПоБюджетам' стала равной:
						...
						| ''                      | '1' | ''           | 'Бюджет движения денежных средств' | "Mercury LLC" | ''           | "Software sale" | ''           | ''           | ''           | ''           | "VA - Main project" | ''                     | 'RUB'    | ''              | '780,000.00'   | '780,000.00'   | ''           |
						| ''                      | '2' | ''           | 'Бюджет движения денежных средств' | "Mercury LLC" | ''           | "Software implementation"  | ''           | ''           | ''           | ''           | "VA - Main project" | ''                     | 'RUB'    | ''              | '2,340,000.00' | '2,340,000.00' | ''           |
		...
		* Создаем новый документ №2
			...
			* Бюджет движения денежных средств
				...
				Если '$$ЭтоЕРПУХ$$' Тогда
					...
					И Я закрываю окно "Cash flow items"
				И в таблице 'ТабБюджетДвиженияДенежныхСредств' из выпадающего списка с именем 'ТабБюджетДвиженияДенежныхСредствДоговорКонтрагента' я выбираю по строке "Io-002"
				И в таблице 'ТабБюджетДвиженияДенежныхСредств' из выпадающего списка с именем 'ТабБюджетДвиженияДенежныхСредствСтатьяДвиженияДенежныхСредств' я выбираю по строке "Software upgrade"
				И в таблице 'ТабБюджетДвиженияДенежныхСредств' из выпадающего списка с именем 'ТабБюджетДвиженияДенежныхСредствЦФО' я выбираю по строке "Mercury LLC"
				И в таблице 'ТабБюджетДвиженияДенежныхСредств' из выпадающего списка с именем 'ТабБюджетДвиженияДенежныхСредствПроект' я выбираю по строке "VA - Main project"
