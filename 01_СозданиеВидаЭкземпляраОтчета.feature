...
Сценарий: 01.03 Работа в контрукторе отчета - Настройка структуры отчета
	...
	* Редактируем свойства показателей
		...
		И из выпадающего списка с именем 'ВидИтога' я выбираю точное значение "Amount"
	...
	* Сравниваем с эталоном
		Тогда табличный документ 'ПолеТабличногоДокументаМакет' равен по шаблону:
			...
			| ''                       | ''                 | 'Код'                   | "LineDimension1_Quantity1*"       | "LineDimension1_Price1*"       | "LineDimension1_Amount1*"       | ''               |
			| "LineDimension2"       | "LineDimension2*"       | ''                      | '■'                            | '■'                      | '■'                       | ''               |
			| ''                       | ''                 | 'Код'                   | "LineDimension2_Quantity1*"       | "LineDimension2_Price1*"       | "LineDimension2_Amount1*"       | ''               |
			| "LineDimension3"       | "LineDimension3*"       | ''                      | '■'                            | '■'                      | '■'                       | ''               |
			| ''                       | ''                 | 'Код'                   | "LineDimension3_Quantity1*"       | "LineDimension3_Price1*"       | "LineDimension3_Amount1*"       | ''               |
			| "LineDimension4"       | "LineDimension4*"       | ''                      | '■'                            | '■'                      | '■'                       | ''               |
			| ''                       | ''                 | 'Код'                   | "LineDimension4_Quantity1*"       | "LineDimension4_Price1*"       | "LineDimension4_Amount1*"       | ''               |
			| "LineDimension5"       | "LineDimension5*"       | ''                      | '■'                            | '■'                      | '■'                       | ''               |
			| ''                       | ''                 | 'Код'                   | "LineDimension5_Quantity1*"       | "LineDimension5_Price1*"       | "LineDimension5_Amount1*"       | ''               |
			| "LineDimension6"       | "LineDimension6*"       | ''                      | '■'                            | '■'                      | '■'                       | ''               |
			| ''                       | ''                 | 'Код'                   | "LineDimension6_Quantity1*"       | "LineDimension6_Price1*"       | "LineDimension6_Amount1*"       | ''               |
			| "LineDimension6Currency" | "LineDimension6Curren*" | ''                      | '■'                            | '■'                      | '■'                       | ''               |
			| ''                       | ''                 | 'Код'                   | "LineDimension6Curren_Quantity1*" | "LineDimension6Curren_Price1*" | "LineDimension6Curren_Amount1*" | ''               |
			| "LineDimensionCurrency"  | "LineDimensionCurrenc*"  | ''                      | '■'                            | '■'                      | '■'                       | ''               |
			| ''                       | ''                 | 'Код'                   | "LineDimensionCurrenc_Quantity1*"  | "LineDimensionCurrenc_Price1*"  | "LineDimensionCurrenc_Amount1*"  | ''               |
...
Сценарий: 01.06 Работа в контрукторе отчета - Ввод формул
	...
	* Вводим формулу вручную - 'ГруппаСтрок - Цена'
		...
		И в табличном документе 'ПолеТабличногоДокументаМакет' я делаю двойной клик на текущей ячейке
		И в поле с именем 'ПолеТекстовогоДокументаПроцедура' я ввожу текст ' '
		И в поле с именем 'ПолеТекстовогоДокументаПроцедура' я ввожу текст "({VAManualEntryForAllAnalytics.LineSynthetic_Price1}+{VAManualEntryForAllAnalytics.LineDimension1_Price1}+{VAManualEntryForAllAnalytics.LineDimension2_Price1}+{VAManualEntryForAllAnalytics.LineDimension3_Price1}+{VAManualEntryForAllAnalytics.LineDimension4_Price1}+{VAManualEntryForAllAnalytics.LineDimension5_Price1}+{VAManualEntryForAllAnalytics.LineDimension6_Price1}+{VAManualEntryForAllAnalytics.LineDimension6Curren_Price1}+{VAManualEntryForAllAnalytics.LineDimensionCurrenc_Price1})/9"
		И я нажимаю на кнопку с именем 'ЗаписатьИСвернуть'
	...
	* Редактируем формулу через форму источников данных
		* Добавляем операнд "Количество"
			...
			И Я выбираю показатель с кодом "LineDimension6Curren_Quantity1"
			Тогда открылось окно "VA - Manual entry for all analytics_LineDimension1 quantity (Data source) *"
			И я нажимаю на кнопку с именем 'ФормаЗаписатьИЗакрыть'
...
Сценарий: 01.08 Создание экземпляра отчета
	...
	* Вводим значения в таблицу
		...
		И Я добавляю значения с раскрытием показателей в ячейку 'R10C2'
			| "VA0ProCate"           | "VA0Product"                | 'Значение'    |
		...
		И Я добавляю значения с раскрытием показателей в ячейку 'R13C2'
			| "VA0ProCate"           | "VA0Product"                | "VA0Counter"      | 'Значение'    |
		...
		И Я добавляю значения с раскрытием показателей в ячейку 'R17C2'
			| "VA0ProCate"           | "VA0Product"                | "VA0Counter"      | "VA0Contrac"  | 'Значение'    |
	...
	* Добавляем Комментарии ячеек
		...
		И я нажимаю на кнопку с именем 'ПолеТабличногоДокументаМакетКомментироватьПоказатель'
		Тогда открылось окно "Comments"
		И в поле с именем 'НовыйКомментарий' я ввожу текст "Comment Synthetic"
		И я нажимаю на кнопку с именем 'ЗаписатьКомментарий'
		Тогда открылось окно '$ЗаголовокОкна$'
		И в табличном документе 'ПолеТабличногоДокументаМакет' я перехожу к ячейке 'R9C4'
		И я нажимаю на кнопку с именем 'ПолеТабличногоДокументаМакетКомментироватьПоказатель'
		Тогда открылось окно "Comments"
		И в поле с именем 'НовыйКомментарий' я ввожу текст "Comment Dimension 1"
		И я нажимаю на кнопку с именем 'ЗаписатьКомментарий'
		Тогда открылось окно '$ЗаголовокОкна$'
		И в табличном документе 'ПолеТабличногоДокументаМакет' я перехожу к ячейке 'R12C5'
		И я нажимаю на кнопку с именем 'ПолеТабличногоДокументаМакетКомментироватьПоказатель'
		Тогда открылось окно "Comments"
		И в поле с именем 'НовыйКомментарий' я ввожу текст "Comment Dimension 2"
		И я нажимаю на кнопку с именем 'ЗаписатьКомментарий'
		Тогда открылось окно '$ЗаголовокОкна$'
		И в табличном документе 'ПолеТабличногоДокументаМакет' я перехожу к ячейке 'R16C6'
		И я нажимаю на кнопку с именем 'ПолеТабличногоДокументаМакетКомментироватьПоказатель'
		Тогда открылось окно "Comments"
		И в поле с именем 'НовыйКомментарий' я ввожу текст "Comment Dimension 3"
		И я нажимаю на кнопку с именем 'ЗаписатьКомментарий'
		Тогда открылось окно '$ЗаголовокОкна$'
		И в табличном документе 'ПолеТабличногоДокументаМакет' я перехожу к ячейке 'R21C7'
		И я нажимаю на кнопку с именем 'ПолеТабличногоДокументаМакетКомментироватьПоказатель'
		Тогда открылось окно "Comments"
		И в поле с именем 'НовыйКомментарий' я ввожу текст "Comment Dimension 4"
		И я нажимаю на кнопку с именем 'ЗаписатьКомментарий'
		Тогда открылось окно '$ЗаголовокОкна$'
		И в табличном документе 'ПолеТабличногоДокументаМакет' я перехожу к ячейке 'R27C8'
		И я нажимаю на кнопку с именем 'ПолеТабличногоДокументаМакетКомментироватьПоказатель'
		Тогда открылось окно "Comments"
		И в поле с именем 'НовыйКомментарий' я ввожу текст "Comment Dimension 5"
		И я нажимаю на кнопку с именем 'ЗаписатьКомментарий'
		Тогда открылось окно '$ЗаголовокОкна$'
