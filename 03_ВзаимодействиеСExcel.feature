...
Сценарий: 03.02 Создание внешней информационной базы "VA - Import from Excel"
	...
	И Я создаю внешнюю информационную базу "VA - Import from Excel" тип 'Файлы Microsoft Excel' каталог "$КаталогПроекта$\Макеты" тип хранилища ''
	...
	* Тестируем механизм маски имени
		...
		Тогда таблица 'СтруктураИмени' стала равной:
			...
			| '\\'           | "Project"            |
			| '\\'           | "Report type"        |
		Тогда элемент формы с именем 'МаскаПолногоИмени' стал равен "$КаталогПроекта$\Макеты\<Scenario period>\<Scenario>\<Company>\<Project>\<Report type>.xls?"
		Когда открылось окно '$ЗаголовокОкна$'
	...
	* Наставиваем элементы пути	
		...
		И Я добавляю элемент пути к файлам организаций для типа 'ВидОтчета' объект "VA - ImportExcel" значение "ВА_ИмпортExcel"
...
Сценарий: 03.03 Создание многопериодного бланка отчета и выгузка в Excel
	...
	* Сохраняем бланк
		...
		И из выпадающего списка с именем 'ВнешняяИБ' я выбираю по строке "VA - Import from Excel"
...
Сценарий: 03.04 Создание экземпляра отчета - "VA - ImportExcel"	и загрузка из файла
	...
	* Документ должен быть пуст
		Дано Табличный документ 'ПолеТабличногоДокументаМакет' равен макету "$КаталогПроекта$\Макеты\ВА_ИмпортExcel_Пустой.mxl"
	...
	* Загружаем ошибочный файл
		И я нажимаю на кнопку с именем 'ФормаЗаполнитьПрочимСпособом'		
		Когда открылось окно "Select method to calculate indicators"
		Тогда у элемента формы с именем 'СпособФормированияОтчета' текст редактирования стал равен "Import"
		Тогда у элемента формы с именем 'БланкИмпорта' текст редактирования стал равен "VA - ImportExcel"
		И в поле с именем 'ПутьКФайлу' я ввожу текст "$КаталогПроекта$\Макеты\ВА_ИмпортExcel+Error.xlsx"
		И я нажимаю на кнопку с именем 'ФормаВыбрать'		
	...
	* Загружаем с файлом
		...
		Тогда у элемента формы с именем 'СпособФормированияОтчета' текст редактирования стал равен "Import"
		Тогда у элемента формы с именем 'БланкИмпорта' текст редактирования стал равен "VA - ImportExcel"
		Тогда у элемента формы с именем 'ФайлИмпорта' текст редактирования стал равен "ВА_ИмпортExcel+Error.xlsx"
		И я меняю значение переключателя с именем 'СпособИмпортирования' на "File on hard drive"
		Тогда у элемента формы с именем 'ПутьКФайлу' текст редактирования стал равен "$КаталогПроекта$\Макеты\ВА_ИмпортExcel+Error.xlsx"
		И в поле с именем 'ПутьКФайлу' я ввожу текст "$КаталогПроекта$\Макеты\ВА_ИмпортExcel+Mercury.xlsx"
		И я нажимаю на кнопку с именем 'ФормаВыбрать'
		...
		Тогда открылось окно '$ЗаголовокОкна$'
		Дано Табличный документ 'ПолеТабличногоДокументаМакет' равен макету "$КаталогПроекта$\Макеты\ВА_ИмпортExcel_Меркурий.mxl"
	...
	* Настраиваем Регламент
		...
		И из выпадающего списка с именем 'СпособФормированияОтчета' я выбираю точное значение "Import"
		И из выпадающего списка с именем 'ВнешнийИсточник' я выбираю по строке "VA - Import from Excel"
		Тогда элемент формы с именем 'БланкИмпорта' стал равен "VA - ImportExcel"				
...
Сценарий: 03.05 Повторная загрузка данных из файла
	...
	* Грузим файл повторно
		...
		И элемент формы с именем 'ФайлИмпорта' стал равен "ВА_ИмпортExcel+Mercury.xlsx"
		И элемент формы с именем 'БланкИмпорта' стал равен "VA - ImportExcel"
		И я нажимаю на кнопку с именем 'ФормаВыбрать'
	...
	* Вибираем второй файл
		...
		Когда открылось окно "Select method to calculate indicators"
		И я меняю значение переключателя с именем 'СпособИмпортирования' на "File on hard drive"
		И в поле с именем 'ПутьКФайлу' я ввожу текст "$КаталогПроекта$\Макеты\ВА_ИмпортExcel+Mercury_2.xlsm"
		И я нажимаю на кнопку с именем 'ФормаВыбрать'
		Тогда открылось окно '$ЗаголовокОкна$'
		Дано Табличный документ 'ПолеТабличногоДокументаМакет' равен макету "$КаталогПроекта$\Макеты\ВА_ИмпортExcel_Меркурий_2.mxl"
	
	* Заполняем из внешней базы
		И я нажимаю на кнопку с именем 'ФормаЗаполнитьПрочимСпособом'
		Когда открылось окно "Select method to calculate indicators"
		И я меняю значение переключателя с именем 'СпособИмпортирования' на "External infobase"
		И из выпадающего списка с именем 'ИспользуемаяИБИмпорт' я выбираю по строке "VA - Import from Excel"
		И я нажимаю на кнопку с именем 'ФормаВыбрать'
		Тогда открылось окно '$ЗаголовокОкна$'
		Дано Табличный документ 'ПолеТабличногоДокументаМакет' равен макету "$КаталогПроекта$\Макеты\ВА_ИмпортExcel_Меркурий.mxl"									
...
Сценарий: 03.06 Создаем бланк сводной таблицы
	...
	* Настраиваем сортировку аналитики Номенклатура
		...
		И элемент формы с именем 'НастройкиКДНастройкиПорядок' стал равен "Product range.Product ID"
...
Сценарий: 03.07 Создаем сводную таблицу
	...
	* Сверяем таблицу
		Когда открылось окно '$ЗаголовокОкна$'
		Дано Табличный документ 'ПолеТабличногоДокументаМакет' равен макету "$КаталогПроекта$\Макеты\ВА_ИмпортExcel_Меркурий_2_СТ.mxl"
	...
	* Загружаем по регламенту
		...
		Дано Табличный документ 'ПолеТабличногоДокументаМакет' равен макету "$КаталогПроекта$\Макеты\ВА_ИмпортExcel_Меркурий_СТ.mxl"	
