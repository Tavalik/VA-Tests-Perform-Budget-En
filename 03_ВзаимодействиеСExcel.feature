...
Сценарий: 03.05 Повторная загрузка данных из файла
	...
	* Грузим файл повторно
		...
		И элемент формы с именем 'ФайлИмпорта' стал равен 'ВА_ИмпортExcel_Меркурий*' по шаблону
	...
	* Вибираем второй файл
		И я нажимаю на кнопку с именем 'ФормаЗаполнитьПрочимСпособом'
		Когда открылось окно "Select method to calculate indicators"	
		И я меняю значение переключателя с именем 'СпособИмпортирования' на "on hard drive"						
		И в поле с именем 'ПутьКФайлу' я ввожу текст "$КаталогПроекта$\Макеты\ВА_ИмпортExcel_Меркурий_2_En.xlsm"
		И я нажимаю на кнопку с именем 'ФормаВыбрать'
